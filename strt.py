...
class IdiomindIndicator:
    ...
    def __init__(self):
        self.indicator = appindicator.Indicator.new(icon, icon, appindicator.IndicatorCategory.OTHER)
    ...
    def make_menu_items(self):
        menu_items = []
        menu_items.append(("New", self.on_Add_click))
        if self.stts == 0:
    ...
    def on_Home(self, widget):
        os.system("python3 '/usr/share/sniparse/main.py' &")

    def on_Add_click(self, widget):
        os.system("python3 '/usr/share/sniparse/add.py' &")
...
if __name__ == "__main__":
    i = IdiomindIndicator()
    gdir = gio.File.new_for_path("/tmp/")
